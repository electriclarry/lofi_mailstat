...
GetOptions(
	...
	"bounce_detail_length=i"   => \$opts{'bounceDetailLength'},
    "d=s"                      => \$opts{'d'},
    "deferral_detail=i"        => \$opts{'deferralDetail'},	
	"deferral_detail_length=i" => \$opts{'deferralDetailLength'},
...
#set length of bounce and deferrer error messages
$opts{'bounceDetailLength'} = 66 unless(defined($opts{'bounceDetailLength'}));
$opts{'deferralDetailLength'} = 65 unless(defined($opts{'deferralDetailLength'}));
...
while(<>) {
		...
		/to=<([^>]*)>, (?:orig_to=<[^>]*>, )?relay=([^,]+), (?:conn_use=[^,]+, )?delay=([^,]+), (?:delays=[^,]+, )?(?:dsn=[^,]+, )?status=(\S+)(.*)$/o) >= 4)
	{
		...
		unless($opts{'deferralDetail'} == 0) {
			...
			# trim the deferral status message to defined length. 
			$deferredReas = said_string_trimmer($deferredReas, $opts{'deferralDetailLength'} );
		...
		unless($opts{'bounceDetail'} == 0) {
		    ...
		    unless(defined($opts{'verbMsgDetail'})) {
			$bounceReas = said_string_trimmer($bounceReas, $opts{'bounceDetailLength'} );
			$bounceReas =~ s/^\d{3} //o;
		    }
